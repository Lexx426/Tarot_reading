...
function MobileNav() {
  ...
  return (
    <div className="flex items-center justify-center border-b border-gray-400 py-8">
        <h2 className='text-3xl md:hidden'>Tarot Temple</h2>
        <nav >
            <section className='flex md:hidden'>
            
                <div className='space-y-3 absolute top-0 right-0 px-8 py-8 cursor-pointer' onClick={() => setMobileNav((prev) => !prev)}>
                    <span className='block w-10 h-0.5 animate-bounce bg-red-400'></span>
                    <span className='block w-10 h-0.5 animate-bounce bg-red-400'></span>
                    <span className='block w-10 h-0.5 animate-bounce bg-red-400'></span>
                </div>
                <div className={MobileNav ? "showMenuNav" : "hideMenuNav"} >
                    <div className="absolute top-0 right-0 px-8 py-8 cursor-pointer" onClick={() => setMobileNav(false)}>
                        <svg
